...
st.write("Versão 3.0 - Processamento Seguro de JSON Ativado")
...
if os.path.exists(NOME_ARQUIVO_BANCO):
    try:
        ...
        if isinstance(dados, dict):
            # Procura por chaves comuns de exportação como 'dados', 'itens', 'resultado'
        ...
        # MAPEAMENTO DAS COLUNAS REAIS (Pegando apenas o essencial para evitar os objetos complexos)
        colunas_necessarias = {}
        for col in df_bruto.columns:
            col_lower = col.lower()
            if 'codigosinterno' in col_lower or 'codigointerno' in col_lower:
                colunas_necessary['codigosInterno'] = col
...
with aba_auditoria:
    ...
    if arquivo_invoice is not None and df_siscomex is not None:
        # Pula as 13 linhas, lê cabeçalho na 14 e limita a 80 linhas para ignorar rodapé
        ...
        # CRUZAMENTO DOS DADOS (O 'Mesclar' perfeito)
